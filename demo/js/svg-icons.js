...
org_tamina_html_component_HTMLApplication.prototype = {
	loadComponents: function() {
		...
		while( $it0.hasNext() ) {
			...
			componentData = this2.get(tag);
			var componentClass = Type.resolveClass(componentData.className);
			if(componentData.parent != null) customElements.define(tag,componentClass,componentData.parent); else customElements.define(tag,componentClass,null);
		}
...
var org_tamina_html_component_HTMLComponent = function() {
	...
	this.observedAttributes.push("icon");
...
svgicons_html_SVGIcon.prototype = $extend(org_tamina_html_component_HTMLComponent.prototype,{
	attributeChangedCallback: function(attrName,oldVal,newVal) {
		...
		if(attrName == "icon") this.loadIcon(newVal);
	}
	,loadIcon: function(iconName) {
		...
		} else if(iconName != "default") this.loadIcon("default");
	}
});
$hxClasses.Math = Math;
...
svgicons_html_SVGIcon.__registered = (function($this) {
	var $r;
	{
		...
		this1.set(key,{ className : "svgicons.html.SVGIcon", parent : null});
	}
	$r = true;
	return $r;
}(this));
